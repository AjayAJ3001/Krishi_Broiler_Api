...
exports.addPO = async (req, res) => {
    const { po_no, supplier__id, bill_no, rr_no, materials, user_id, status } = req.body;
    ...
    try {
        const queryText = `
            INSERT INTO PO (po_no, supplier__id, bill_no, rr_no, materials, user_id, status, created_at, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            RETURNING *;
        `;

        const values = [po_no, supplier__id, bill_no, rr_no, JSON.stringify(materials), user_id, status];
...
exports.updatePO = async (req, res) => {
    const { id } = req.params;
    const { po_no, supplier__id, bill_no, rr_no, materials, status } = req.body;
    ...
    try {
        ...
        const queryText = `
            UPDATE PO 
            SET po_no = $1, supplier__id = $2, bill_no = $3, rr_no = $4, materials = $5, status=$6, updated_at = CURRENT_TIMESTAMP
            WHERE id = $7
            RETURNING *;
        `;

        const values = [po_no, supplier__id, bill_no, rr_no, JSON.stringify(materials), status, id];
