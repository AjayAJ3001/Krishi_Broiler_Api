function convertToSAP(item) {
    ...
    return {
        id: item.s_no,
        BLDAT: item.doc_date.toISOString().split("T")[0],
        XBLNR: item.doc_no,
        ZTO: item.to,
        ...
        AEDAT: item.doc_date.toISOString().split("T")[0],
        ZRR_DATE: item.doc_date.toISOString().split("T")[0],
        ZSUPP_NAME: item.supplier_name,
        ZSUP_INV_DATE: item.doc_date.toISOString().split("T")[0],
        LFUHR: item.doc_date.toISOString().split("T")[0],
        MEINS: item.unit_name,
        FKART: item.doc_no?.includes("T") ? "Taxable" : "Exempted",
        FKDAT: item.doc_date.toISOString().split("T")[0],
