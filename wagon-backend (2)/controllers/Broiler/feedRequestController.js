...
const generateFeedTransferDC = async (data) => {
    try {
        const { freight, mobile, vehicleNumber, requests} = data;
        const user_id = "test"
        ...
        const templateData = {
            doc_no, freight, mobile, vehicle_no:vehicleNumber, requests
        };

        // 2. Render HTML using EJS
        const templatePath = path.join(process.cwd(), 'templates', 'broiler', 'feed_request_dc.ejs');
        const htmlContent = await ejs.renderFile(templatePath, templateData);

        // 3. Setup File Path & Directories
        const reportsDir = path.join(process.cwd(), 'uploads', 'broiler', 'feed_request');
        ...
        // 5. Save to Database
        const insertQuery = `
            INSERT INTO broiler.${broilerDataEntry[TABLE_NAME].pgTable} ( doc_no, freight, mobile, vehicle_no, requests,  user_id, pdf_url
            ) VALUES ($1, $2, $3, $4, $5, $6, $7)
        `;

        const values = [
            doc_no, freight, mobile, vehicleNumber, JSON.stringify(requests), user_id, publicUrl
        ]
